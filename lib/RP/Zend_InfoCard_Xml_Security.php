...
class Zend_InfoCard_Xml_Security
{
     ...
     * @return string String if the signature validated, false otherwise
     * @throws Exception
     */
     ...
     * @return string|false
     */
    static private function addNamespace($xmlElem, $ns)
    {
        $schema = '.*<[^<]*SignedInfo[^>]*'.$ns.'.*>.*';
        $pattern = '/\//';
        $replacement = '\/';
        $nspattern = '/'.preg_replace($pattern, $replacement, $schema).'/';
        if (preg_match($nspattern, strval($xmlElem->asXML())) > 0) { //M$ Cardspaces
            $xml = $xmlElem->asXML();
        } else { //Digitalme
            $xmlElem->addAttribute('DS_NS', $ns);
            $xml = $xmlElem->asXML();
            if (preg_match("/<(\w+)\:\w+/", strval($xml), $matches)) {
                $prefix = $matches[1];
                $xml = str_replace("DS_NS", "xmlns:".$prefix, strval($xml));
            } else {
                $xml = str_replace("DS_NS", "xmlns", strval($xml));
            }
        }
        return $xml;
    }
     ...
     * @param int $type The encoding format constant
